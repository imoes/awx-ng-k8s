...
awx-ng (8052)
  └─ awx_ee [local] (port 2222)   ← Receptor Control Node
       ├─ ansible03.ippen.media   ← Remote Proxy (Site: MUE-0)
...
ansible-playbook deploy/ansible/install-receptor-proxy.yml \
  -i "ansible03.ippen.media," \
  -e "proxy_node_id=ansible03-ippen-media" \
  -e "awx_ng_control_host=awx-ng.ippen.media" \
...
---
- node:
    id: ansible03-ippen-media

- log-level: info

- tcp-peer:
    address: awx-ng.ippen.media:2222
    redial: true
...
1. **Administration → Instances** → neuer Execution Node mit `hostname=ansible03-ippen-media`
...
|---------------------------|----------------------------|------------|
| ansible03.ippen.media     | ansible03-ippen-media      | MUE-0      |
