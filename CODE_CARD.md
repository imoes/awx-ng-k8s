...
- **Layout: nur innere Panels scrollen, nie die ganze Seite**: die 3-Spalten-Zeile (Canvas/YAML/Variablen) bekommt eine Höhe, die exakt den verbleibenden Viewport unterhalb ausfüllt — berechnet via `getBoundingClientRect().top` in einem `useLayoutEffect` (`builderRowRef`/`builderRowHeight` in `PlaybookBuilder.js`), neu berechnet bei Resize und bei allem, was die Zeile nach oben/unten verschieben könnte (Alerts, Rollen-Reiter). Jede Spalte ist `height:'100%'` mit eigenem `overflow`; Canvas/YAML/Variablen-Spalten teilen sich `flex: 44% / 36% / 220px fix` (vorher 58/26/auto — Canvas bewusst schmaler gemacht, damit die YAML-Spalte nicht mehr horizontal gescrollt werden muss). `CodeEditor` nutzt `rows="auto"` (Höhe nach Inhalt) statt einer festen Zeilenzahl, gewrappt in eine `flex:1, minHeight:0, overflow:'auto'`-Box — bei zu viel Inhalt scrollt nur diese Box. `VariablesPanel.js` füllt ebenso `height:'100%'` statt einer festen `maxHeight:480`.
